...
The datasets are **synthetic**. They're generated by [`scripts/generate-data.ts`](scripts/generate-data.ts) with fixed seeds, so every run gives the same files, and they're shaped like real operational data: seasonality, route and site differences, adoption curves after a go-live. Names of sites, courts and people are anonymised. None of the figures describe a real organisation.

The report previews in `screenshots/` are rendered from these same CSVs by [`scripts/build-dashboards.ts`](scripts/build-dashboards.ts), so every number in an image can be traced to the data and checked with the SQL in each project.

## Rebuild everything

The scripts are TypeScript (`scripts/*.ts`), run with [tsx](https://tsx.is). Row types for every CSV live in [`scripts/types.ts`](scripts/types.ts), shared by the generator and the report builder.

```bash
npm install
npx playwright install chromium   # only needed for screenshots
npm run all                       # typecheck → data → summaries + previews → PNG screenshots
```
